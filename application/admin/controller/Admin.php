...
class Admin extends Base
{
    ...
    public function stop(){
        $id = input('id');
        // var_dump(session('manager_info.id'));
        if($id == session('manager_info.id')){
         return json(['code' => '404', 'msg' => '不能禁用本用户自己！']); 
        }
        \app\admin\model\Admin::update(['status'=>0],['id'=>$id]);
        return json(['code' => '200', 'msg' => '操作成功']);
    }
    //修改管理员状态
    public function start(){
        $id = input('id');
        \app\admin\model\Admin::update(['status'=>1],['id'=>$id]);
        return json(['code' => '200', 'msg' => '操作成功']);
    }
    //addElasticsearch
    public function add_ela(){
    ...
    public function edit_ela(){
        ...
        $params = [
            ...
            'id'    => 100,
            'body'  => [
                'doc' => ['doc' => 100,'title' => 'ES从入门到精通' , 'author' =>'张三']
            ]
        ];
        $r = $es->update($params);
        dump($r);die;
