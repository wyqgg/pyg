...
class Goods extends Base
{
    ...
    public function index()
    {
        ...
        // $list = \app\admin\model\Goods::with('category')->where($where)->order('id desc')->paginate(2, false, $config);
        // var_dump($list);
        foreach($list as &$v){
...
//        $data = request()->param();
        ...
        try{
            ...
            $spec = \app\admin\model\SpecGoods::create($spec_goods,true);
            \think\Db::commit();
        }catch(\Exception $e){
            \think\Db::rollback();
            $msg = $e->getMessage();
    ...
    {
        //查询原始商品数据,category.brands,type.specs,type.specs.spec_values
        $info = \app\admin\model\Goods::with('category,category.brand,brand,type,type.spec,type.spec.spec_value,goods_images,spec_goods')->find($id);
        $info = json_decode(json_encode($info), true);
        $info['type']['attrs'] = \app\common\model\Attribute::where('type_id', $info['type_id'])->select();
        
        foreach ($info['type']['attrs'] as $k=>&$v){
            // var_dump($v['attr_values']);
           $v['attr_values'] = unserialize($v['attr_values']);
        }
        // die;
        unset($v);
        ...
        foreach($info['spec_goods'] as $k=>&$v){
            $v['spec_values'] = \app\common\model\SpecValue::select(explode('_', $v['value_ids']));
        }
        unset($v);
        ...
        // dump($data['category']['brand']);die;
        
        // var_dump($data['goods']['type']['attrs'][0]['attr_values']);die;
