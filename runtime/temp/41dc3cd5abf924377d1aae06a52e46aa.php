<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:73:"/www/wwwroot/pyg/public/../application/admin/view/goods/product-edit.html";i:1620639134;}*/ ?>
<!--_meta 作为公共模版分离出去-->
<!DOCTYPE HTML>
<html>
<head>
					...
					<div class="formControls col-xs-8 col-sm-9">
						<span class="select-box" style="width:150px;">
							<select name="brand_id" class="select" size="1" id="brand">
								...
								<?php foreach($goods['category']['brand'] as $v): ?>
					...
					<div id="ajax_spec_data" class="col-xs-8">
						<table class="table table-border table-bordered table-bg table-sort" id="goods_spec_table1">
							<tbody>
								...
								<?php foreach($goods['type']['spec'] as $v): ?>
								<tr class="spec_name" spec_id="<?php echo $v['id']; ?>">
									...
									<td >
										...
										<button type="button" spec_value_id="<?php echo $value['id']; ?>" class="btn <?php if(in_array(($value['id']), is_array($goods['spec_goods'][0]['value_ids'])?$goods['spec_goods'][0]['value_ids']:explode(',',$goods['spec_goods'][0]['value_ids']))): ?>btn-success<?php else: ?>btn-default<?php endif; ?>"><?php echo $value['spec_value']; ?></button>
										<?php endforeach; ?>
									</td>
								</tr>
								<?php endforeach; ?>
